...
export const followExisting = async (
  follower_id: number,
  following_id: number,
) => {
...
//RESPOSTS
export const repostPost = async (user_id: number, post_id: number) => {
  return await prisma.repost.create({
    data: { user_id, post_id },
  });
};

export const deleteRepost = async (user_id: number, post_id: number) => {
  return await prisma.repost.delete({
    where: { user_id_post_id: { user_id, post_id } },
  });
};
export const repostExisting = async (user_id: number, post_id: number) => {

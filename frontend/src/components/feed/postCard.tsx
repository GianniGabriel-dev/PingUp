...
export const IndividualPost = (post: Post) => {
  ...
  const shouldShowTranslateButton =
    post.language && userLanguage && post.language !== userLanguage;
  // en base al estado de showOriginal muestra el texto traducido o el del post original
  const contentToDisplay =
    displayText && showOriginal ? displayText : post.content;
        ...
        <div className="flex flex-col w-full ">
          ...
          {post.repostedBy && post.repostedBy.length > 0 && (
            <div className="flex items-center gap-1 ml-1 mb-1 text-gray-500 text-sm">
              <RetweetIcon2 size={16} />
              <Link
                to={`/${post.repostedBy[0].user.username}`}
                className="hover:underline"
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`/${post.repostedBy[0].user.username}`);
                }}
              >
          ...
          <footer className="w-full flex justify-between mt-2 text-gray-500">
            ...
            <LikeButton
              ...
              initialIsLiked={post.likes ? post.likes.length > 0 : false}
            />
            <RetweetButton
              postId={post.id}
